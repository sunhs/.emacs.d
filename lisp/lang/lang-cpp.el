...
(setq cc-hook-map
  '((c++-mode-hook c++-mode-map) (c-mode-hook c-mode-map)))

(dolist (hook-map cc-hook-map)
  (lexical-let
    ((hook (car hook-map)) (mmap (car (cdr hook-map))))
    (add-hook hook
      #'
      (lambda ()
        (define-key (symbol-value mmap) (kbd "M-o")
          'hs-cmd/find-other-file-in-project)
